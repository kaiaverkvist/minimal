...
import (
	...
	"github.com/kaiaverkvist/minimal/pkg/database"
	"github.com/kaiaverkvist/minimal/pkg/provider"
	"github.com/kaiaverkvist/minimal/pkg/setup"
...
*/
func New(config Config, routes []provider.Provider, models []any) Server {
	return Server{
		e: echo.New(),
...
func (s *Server) Init(fs http.FileSystem) {
	setup.Logging(s.e, s.config.FriendlyLogging)
	...
	setup.Start(s.e, address, s.config.AutoTLS, s.config.CertKeyPath, s.config.CertPrivateKeyPath, s.config.Domains)
}
